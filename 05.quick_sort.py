def quick_sort(items, start=0, end=None):
    ...
    # 最坏时间复杂度: O(n^2), n+(n-1+1)+(n-2+2)...+(n-n+n) = n*n = O(n^2)
    # 稳定性: 不稳定
    ...
    # 循环移动两个指针
    while low < high:
        # 先将high指针向左移动, 当high指的元素值大于等于key值时, 继续移动
        while low < high and items[high] >= key:
            high -= 1
        # 当high指的元素小于key时, 停止移动, 并将low指的值替换成high指的值
        # 第一次替换时, 是将items[start]替换成high指向的值, 这样就会把items[start]的值覆盖掉了, 而其原值已被key保存了下来
        items[low] = items[high]
        # 替换完之后, low指向的值肯定就小于key了, 这时候就开始移动low
        # 将low指针向右移动, 当low指的元素值小于等于key值时, 继续移动
        while low < high and items[low] <= key:
            low += 1
        # 当low指的元素大于key时, 停止移动, 并将high指的值替换成low指的值
